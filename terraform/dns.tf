...
module "dns_webwhen" {
  ...
  records = [
    # Clerk authentication (webwhen.ai is now Clerk primary domain; instance id 7o0ix56d85sc)
...
# The app chart serves the www → apex redirect at the shared Gateway. This
# avoids requiring Cloudflare Rulesets permission from the DNS automation token.
# The apex A record is owned by ExternalDNS from the app HTTPRoute.
